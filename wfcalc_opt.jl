...
function main()
    ...
    geom, funcArray, MOocc, MOenergy, primMatrix, virial, totalEnergy = readWfn("PhenolDimer.wfn")

    gradScalar, wfn = spaceGradPy(geom, funcArray, primMatrix)
    writedlm("grad.csv", gradScalar[7,:,:], "\t")

    rdg = getRDG(gradScalar, wfn)
    writedlm("rdg.csv", rdg[7,:,:], "\t")
